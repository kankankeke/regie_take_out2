...
//        employee.setUpdateTime(LocalDateTime.now());
        ...
        //获得当前登录用户的id,将getAttribute得到的object类型转为long类型
...
//        employee.setUpdateUser(empId);
    ...
    public Result<Page> page(int page,int pageSize,String name){
        ...
        LambdaQueryWrapper<Employee> queryWrapper = new LambdaQueryWrapper();
        //添加过滤条件
        queryWrapper.like(StringUtils.isNotEmpty(name),Employee::getName,name);//当name不等于空的时候才会加添加，否则这条件相当于不存在
        //添加排序条件
...
//        employee.setUpdateUser(empId);
     ...
     */
    @GetMapping("/{id}")
    public Result<Employee> getById(@PathVariable Long id){//@PathVariable从路径获取参数
              log.info("根据id查询员工信息");
